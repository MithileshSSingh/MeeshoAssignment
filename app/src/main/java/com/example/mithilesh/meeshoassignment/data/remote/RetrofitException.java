...
public class RetrofitException extends RuntimeException {
    ...
    RetrofitException(String message, String url, Response response, Kind kind, Throwable exception, Retrofit retrofit) {
        ...
        try {
            this.errorResponse = getErrorBodyAs(ApiErrorResponse.class);
        } catch (Exception e) {
            e.printStackTrace();
            this.errorResponse = new ApiErrorResponse();
            this.errorResponse.setStatusCode(response.code());
            this.errorResponse.setMessage(response.message());
            try {
                this.errorResponse.setError(response.errorBody().string());
            } catch (IOException e1) {
                e1.printStackTrace();
                this.errorResponse.setError(String.valueOf(response.code()));
            }
        }
    }
